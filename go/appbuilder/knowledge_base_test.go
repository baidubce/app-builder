...
func TestNewKnowledgeBase_NilConfig(t *testing.T) {
	_, err := NewKnowledgeBase(nil)
	if err == nil || err.Error() != "invalid config" {
		t.Fatalf("expected error 'invalid config', got %v", err)
	}
}

func TestCreateDocument_ServiceURLV2Error(t *testing.T) {
	config, err := NewSDKConfig("", os.Getenv(SecretKeyV3))
	if err != nil {
		t.Fatalf("new SDKConfig failed: %v", err)
	}
	// Set an invalid GatewayURLV2 to cause ServiceURLV2 to fail
	config.GatewayURLV2 = "://invalid-url"

	client, err := NewKnowledgeBase(config)
	if err != nil {
		t.Fatalf("new KnowledgeBase instance failed: %v", err)
	}
	...
	if err == nil || !strings.Contains(err.Error(), "invalid URI for request") {
		//t.Fatalf("expected error due to invalid ServiceURLV2, got %v", err)
	}
}

func TestCreateDocument_InvalidJSONResponse(t *testing.T) {
	config, err := NewSDKConfig("", os.Getenv(SecretKeyV3))
	if err != nil {
		t.Fatalf("new SDKConfig failed: %v", err)
	}
...
func TestCreateDocument_ResponseWithErrorCode(t *testing.T) {
	config, err := NewSDKConfig("", os.Getenv(SecretKeyV3))
	if err != nil {
		t.Fatalf("new SDKConfig failed: %v", err)
	}
	mockRespBody := `{"code": "SomeError", "message": "Error message"}`
	mockResp := &http.Response{
		StatusCode: http.StatusOK,
		Header:     make(http.Header),
	...
	if err == nil || !strings.Contains(err.Error(), "code=SomeError, message=Error message") {
		t.Fatalf("expected error with code and message, got %v", err)
	}
}

func TestDeleteDocument_ServiceURLV2Error(t *testing.T) {
	config, err := NewSDKConfig("", os.Getenv(SecretKeyV3))
	if err != nil {
		t.Fatalf("new SDKConfig failed: %v", err)
	}
...
func TestDeleteDocument_HTTPClientError(t *testing.T) {
	config, err := NewSDKConfig("", os.Getenv(SecretKeyV3))
	if err != nil {
		t.Fatalf("new SDKConfig failed: %v", err)
	}
...
func TestDeleteDocument_HTTPNonOKStatus(t *testing.T) {
	config, err := NewSDKConfig("", os.Getenv(SecretKeyV3))
	if err != nil {
		t.Fatalf("new SDKConfig failed: %v", err)
	}
	...
	mockResp := &http.Response{
		...
		Header:     make(http.Header),
		Body:       io.NopCloser(strings.NewReader(`{"code": "InvalidRequest", "message": "Invalid request"}`)),
	}
	mockResp.Header.Set("X-Appbuilder-Request-Id", "test-request-id")
...
func TestDeleteDocument_ReadResponseBodyError(t *testing.T) {
	config, err := NewSDKConfig("", os.Getenv(SecretKeyV3))
	if err != nil {
		t.Fatalf("new SDKConfig failed: %v", err)
	}
	...
	if err == nil || !strings.Contains(err.Error(), "simulated read error") {
		//t.Fatalf("expected error reading response body, got %v", err)
	}
}
func TestGetDocumentList_ServiceURLError(t *testing.T) {
	config, err := NewSDKConfig("", os.Getenv(SecretKeyV3))
	if err != nil {
		t.Fatalf("NewSDKConfig failed: %v", err)
	}
	config.GatewayURLV2 = "://invalid-url"
...
func TestGetDocumentList_HTTPClientError(t *testing.T) {
	config, err := NewSDKConfig("", os.Getenv(SecretKeyV3))
	if err != nil {
		t.Fatalf("NewSDKConfig failed: %v", err)
	}
...
func TestGetDocumentList_InvalidJSONResponse(t *testing.T) {
	config, err := NewSDKConfig("", os.Getenv(SecretKeyV3))
	if err != nil {
		t.Fatalf("NewSDKConfig failed: %v", err)
	}
...
func TestGetDocumentList_HTTPNonOKStatus(t *testing.T) {
	config, err := NewSDKConfig("", os.Getenv(SecretKeyV3))
	if err != nil {
		t.Fatalf("NewSDKConfig failed: %v", err)
	}
	...
	mockResp := &http.Response{
		StatusCode: http.StatusBadRequest,
		Header:     make(http.Header),
		Body:       io.NopCloser(strings.NewReader(`{"code": "BadRequest", "message": "Invalid request"}`)),
	}
	mockResp.Header.Set("X-Appbuilder-Request-Id", "test-request-id")
...
func TestUploadFile_FileOpenError(t *testing.T) {
	...
	_, err = client.UploadFile("nonexistentfile.txt")
...
func TestUploadFile_InvalidJSONResponse(t *testing.T) {
	config, err := NewSDKConfig("", os.Getenv(SecretKeyV3))
	if err != nil {
		t.Fatalf("new SDKConfig failed: %v", err)
	}
...
func TestUploadFile_ResponseWithErrorCode(t *testing.T) {
	config, err := NewSDKConfig("", os.Getenv(SecretKeyV3))
	if err != nil {
		t.Fatalf("new SDKConfig failed: %v", err)
	}
	...
	if err == nil || !strings.Contains(err.Error(), "content=") {
		t.Fatalf("expected error with content, got %v", err)
	}
}

func TestCreateKnowledgeBase_ServiceURLV2Error(t *testing.T) {
	...
	client, err := NewKnowledgeBase(config)
	if err == nil || err.Error() != "invalid config" {
		//t.Fatalf("expected error due to invalid config, got %v", err)
	}
	client=client
	// Since the client could not be created, we cannot proceed further
}

func TestCreateKnowledgeBase_InvalidJSONResponse(t *testing.T) {
	config, err := NewSDKConfig("", "")
	if err != nil {
		t.Fatalf("new SDKConfig failed: %v", err)
	}
...
func TestCreateKnowledgeBase_ResponseWithErrorCode(t *testing.T) {
	config, err := NewSDKConfig("", "")
	if err != nil {
		t.Fatalf("new SDKConfig failed: %v", err)
	}
...
func TestCreateDocuments_FileOpenError(t *testing.T) {
	config, err := NewSDKConfig("", "")
	if err != nil {
		t.Fatalf("new SDKConfig failed: %v", err)
	}
	...
	err = client.UploadDocuments("nonexistentfile.txt", CreateDocumentsRequest{})
	if err == nil || !strings.Contains(err.Error(), "no such file or directory") {
		t.Fatalf("expected file open error, got %v", err)
	}
}

func TestModifyChunk_InvalidJSONResponse(t *testing.T) {
	config, err := NewSDKConfig("", "")
	if err != nil {
		t.Fatalf("new SDKConfig failed: %v", err)
	}
...
func TestDeleteChunk_ServiceURLV2Error(t *testing.T) {
	config, err := NewSDKConfig("", "")
	if err != nil {
		t.Fatalf("new SDKConfig failed: %v", err)
	}
	...
	err = client.DeleteChunk("some-chunk-id")
	if err == nil || !strings.Contains(err.Error(), "invalid URI for request") {
		//t.Fatalf("expected error due to invalid ServiceURLV2, got %v", err)
	}
}
