...
class PPTGenerationFromInstruction(Component):
    ...
    def run(self, message: Message, poll_request_times=60, poll_request_interval=5) -> Message:
        ...
        if 'custom_data' in user_input and user_input['custom_data']:
    ...
    def tool_eval(self, text: str, **kwargs):
        ...
        result = self.run(message,
                          poll_request_times=kwargs.get("poll_request_times", 60),
                          poll_request_interval=kwargs.get("poll_request_interval", 5))
        ppt_download_link = result.content

        yield self.create_output(type="text", text='已成功为您生成PPT！', visible_scope='llm', name="text")
        yield self.create_output(type="urls", text=ppt_download_link, name="urls")