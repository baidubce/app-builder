...
class StyleWriting(CompletionBaseComponent):
    ...
    Examples:
        ...
        .. code-block:: python
            ...
            style_writing = appbuilder.StyleWriting(model="eb-turbo-appbuilder")
    ...
    def __init__(self, model=None):
        ...
        """
        super().__init__(StyleWritingArgs, model=model)

    def run(self, message, style_query="通用", length=100, stream=False, temperature=1e-10):
        """
        使用给定的输入运行模型并返回结果。
        ...
        """
        return super().run(message=message, style_query=style_query, length=length, stream=stream, temperature=temperature)
