...
class TreeMind(Component):
    ...
    def _post(self, query, **kwargs):
        ...
        response = self.http_client.session.post(tree_mind_url, headers=headers, data=payload)
        self.http_client.check_response_header(response)
        data = response.text
        treemind_dict = json.loads(data.split("data:")[-1])
        jump_link = treemind_dict['info']['downloadInfo']['fileInfo']['jumpLink']
        img_link = treemind_dict['info']['downloadInfo']['fileInfo']['pic']
        return img_link, jump_link
        
    @components_run_stream_trace
    async def a_tool_eval(
            self,
            query,
            **kwargs,
    ):
    ...
    async def arun(self, message: Message, **kwargs) -> Message:
        query = message.content
        img_link, jump_link = self._post(query, **kwargs)

        result = {
            "result": "生成的思维导图：![图片url]({img_link})思维导图已经为您生成好了，如果您觉得这个思维导图还不够完美，或者您的想法需要更自由地表达，点击编辑按钮，对思维导图变形、变色、变内容、甚至可以添加新的元素，您可以通过这个链接编辑：{jump_link}。".format(img_link=img_link, jump_link=jump_link),
            "img_link": img_link,
            "edit_link": jump_link
        }
