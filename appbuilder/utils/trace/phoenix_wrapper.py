...
def launch_phoenix(host: Optional[str] = "127.0.0.1", port: Optional[int] = 8080, **kwargs):
    ...
    try:
        ...
        raise (
            "尚未安装phoenix组件，尝试使用 python3 -m pip install arize-phoenix -i https://pypi.tuna.tsinghua.edu.cn/simple 安装该组件")
    session = phoenix.launch_app(host=host, port=port, **kwargs)
    return session
